...
const Hero = ({ lang }: { lang: Lang }) => {
  ...
  return (
    <>
      <div className="bg-white/30 backdrop-blur-sm p-6 md:p-12 rounded-lg border border-white/20 shadow-xl max-w-3xl w-full">
        ...
        <p className="text-lg md:text-xl lg:text-2xl tracking-wide text-black drop-shadow-md">
          {t.dateLine}
        </p>

        <p className="mt-6 text-black max-w-xl mx-auto text-sm md:text-base lg:text-lg">
          {t.description}
        </p>

        {/* Countdown */}
        <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4 text-black">
          {[
            {
            ...
            >
              ...
              <div className="mt-1 text-[clamp(0.55rem,1.2vw,0.75rem)] uppercase tracking-[0.15em] opacity-70 whitespace-nowrap overflow-hidden text-ellipsis">
                {item.label}
              </div>
            </div>
          ))}
